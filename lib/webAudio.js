...
module.exports = (function() {
      ...
      return function() {
        ...
        for(let _deviceId in audioDeviceObject.audioNode.outputNodes) {
          ...
          try {
            ...
            destDeviceObject.write(audioDeviceObject.audioBuffer, audioDeviceObject);
  ...
  */
  ...
  class incomingAudioTrackMixer extends nonBrowserAudioNode {
    constructor(data) {
      super(data);
      ...
      this.getDeviceObject().write = (float32Samples, sourceAudioDeviceObject) => {
        const focusUserId = storeLib.get('applicationState')['focusUserId'] || '';
        const userId = sourceAudioDeviceObject.name.split('_')[0];
        const shouldAllowAudioStream = (userId == focusUserId);
        if (shouldAllowAudioStream) {
  ...
  class SilenceDetector {
    constructor({ sampleRate, isSilentWhenPowerIsBelow }) {
      this.sampleRate = sampleRate;
      this.isSilentWhenPowerIsBelow = isSilentWhenPowerIsBelow;
      const howlongShouldPowerStayLowForSilenceDetectionSeconds = 2;
      this.howmanySamplesShouldPowerStayLowForSilenceDetection = this.sampleRate * howlongShouldPowerStayLowForSilenceDetectionSeconds;
      this.totalConsequentiveSilenceSamples = 0;
    }
    onNewSample(float32Samples) {
      ...
      if (avgPower < this.isSilentWhenPowerIsBelow) {
        this.totalConsequentiveSilenceSamples += float32Samples.length;
        if (this.totalConsequentiveSilenceSamples > this.howmanySamplesShouldPowerStayLowForSilenceDetection) {
          silenceDetected = true;
        }
      } else {
        this.totalConsequentiveSilenceSamples = 0;
      }
      return { avgPower, silenceDetected };
    }
  }
  ...
  class nonBrowserSocketWriterNode extends nonBrowserAudioNode {
    constructor(data) {
      ...
      // TODO: move the logic to silenceDetector class
      const howlongShouldPowerStayLowForSilenceDetectionSeconds = 2;
      ...
      audioDeviceObject.write = float32Samples => {
        ...
        // placeholder - only place we calc avgPower
        // may be we should always calc power regardless of whether 
  ...
  class nonBrowserSpeakerNode extends nonBrowserAudioNode {
    constructor(data) {
      ...
      if (data.stopStreamingWhenSilent) {
        this.silenceDetector = new SilenceDetector({
          sampleRate: this.getDeviceObject().streamProperties.sampleRate,
          isSilentWhenPowerIsBelow: data.isSilentWhenPowerIsBelow
        });
      }
    ...
    start(deviceNameGrepStr) {
      ...
      this.getDeviceObject().write = float32Samples => {
        ...
        let isAllZeroPacket = true;
        for(let i=0; i < float32Samples.length; ++i) {
          if (float32Samples[i] != 0) {
            isAllZeroPacket = false;
            break;
          }
        }
        if (isAllZeroPacket) {
          audioDeviceObject.datastreamMonitor.log({ msg: {
            action: 'dropAllZeroAudioPacket',
            audioSource: audioDeviceObject
          }});
          return;
        }
        ...
        if (this.silenceDetector) {
          const { avgPower, silenceDetected } = this.silenceDetector.onNewSample(float32Samples);
          audioDeviceObject.silenceDetected = silenceDetected;
          audioDeviceObject.avgPower = avgPower;
          if (silenceDetected) {
            float32Samples = new Float32Array(1);
            audioDeviceObject.datastreamMonitor.log({ msg: {
  ...
  class nonBrowserAudioContext {
    ...
    async createNodeFromXCastConfig(queryObject) {
      ...
      switch(config.mode) {
        ...
        case 'OscillatorNode':
          ...
          break;
        case 'IzySpeakerNode':
          waveDeviceNode = this.createIzySpeakerNode(config);
          break;
    ...
    createIncomingAudioTrackMixer() {
      return new incomingAudioTrackMixer({
        deviceId: 'incomingAudioTrackMixer-' + (new Date()).getTime(),
        audioContext: this
      });
    }
    ...
    createIzySpeakerNode(config) {
      const data = Object.assign({
        deviceId: 'SpeakerNode-' + (new Date()).getTime(),
        audioContext: this,
      }, config || {});
      return new nonBrowserSpeakerNode(data);
    }
